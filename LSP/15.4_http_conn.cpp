 ...
 * Author: Andrea
 * Note: 
 * Created Time: 2018年04月10日 星期二 15时43分47秒
 */

#include "15.4_http_conn.h"
...
const char* error_404_form = "Request file was not found on server.\n";
const char* error_500_title = "Internal Error";
const char* error_500_form = "An unusual problem serving the request file.\n";
...
HTTPconn::HTTP_CODE HTTPconn::process_read() {
	...
	while(((m_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK)) || ((line_status = parse_line()) == LINE_OK)) {
		...
		switch(m_check_state) {
			case CHECK_STATE_REQUESTLINE : {											   ret = parse_request_line(text);
											   if(ret == BAD_REQUEST)
												   return BAD_REQUEST;
											   break;
										   }
...
bool HTTPconn::add_content_len(int content_len) {
